...
import { FaHeartbeat, FaStethoscope, FaComments, FaBrain } from 'react-icons/fa';
...
const navLinks = [
    ...
    { to: '/architecture', label: 'Architecture & Algorithm', icon: <FaBrain className="inline mr-1 text-primary-blue" /> },
                ...
                <div className={`flex-1 justify-end items-center space-x-3 font-medium sm:flex hidden`}>
                    {navLinks.map(({ to, label, icon }) => (
                        <NavLink
                            ...
                            to={to}
                            className={({ isActive }) =>
                                `px-2 py-1 rounded-lg transition-all ${isActive ? 'bg-primary-teal text-white' : 'text-gray-700 hover:bg-soft-blue'
                                }`
                            }
                            end={to === '/'}
                        >
            ...
            <div className={`sm:hidden ${open ? 'block' : 'hidden'} px-4 pb-4`}>
                {navLinks.map(({ to, label, icon }) => (
                    <NavLink
                        ...
                        to={to}
                        className={({ isActive }) =>
                            `block px-3 py-2 rounded-md my-1 ${isActive ? 'bg-primary-teal text-white' : 'text-gray-700 hover:bg-soft-blue'
                            }`
                        }
